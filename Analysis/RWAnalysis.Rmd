...
```{r Model comp}
fit2 <- read.csv("Model Fit/OPTIMfits/res_prob_2parms.csv"); 
fit2$parms <- 2; fit2$res <- 0; fit2$sal <- 1e-6; fit2$mem <- 1-1e-6; fit2$zet <- 1e-6; fit2$dlrc <- 1e-10
fit3 <- read.csv("Model Fit/OPTIMfits/res_prob_3parms.csv");
fit3$parms <- 3; fit3$sal <- 1e-6; fit3$mem <- 1-1e-6; fit3$zet <- 1e-6;  fit3$dlrc <- 1e-10
fit4 <- read.csv("Model Fit/OPTIMfits/res_prob_4parms.csv");
fit4$parms <- 4; fit4$mem <- 1-1e-6; fit4$zet <- 1e-6;  fit4$dlrc <- 1e-10
fit5 <- read.csv("Model Fit/OPTIMfits/res_prob_5parm.csv");
fit5$parms <- 5; fit5$zet <- 1e-6;  fit5$dlrc <- 1e-10
fit6 <- read.csv("Model Fit/OPTIMfits/res_prob_6parms.csv");
fit6$parms <- 6; fit6$dlrc <- 1e-10
fit7 <- read.csv("Model Fit/OPTIMfits/res_prob_7parms.csv");
fit7$parms <- 7; 
fitWSLS <- read.csv("Model Fit/OPTIMfits/res_prob_WSLS.csv");
fitWSLS$parms <- "WSLS"; fitWSLS$tau <- 0.01; fitWSLS$lrc <- 0.99; fitWSLS$mem <- 1-1e-6; fitWSLS$zet <- 1e-6; fitWSLS$dlrc <- 1e-10;
fitPH <- read.csv("Model Fit/OPTIMfits/res_prob_PH.csv");
...
FitRealBehav_df <- plyr::join(FitRealBehav, 
                         probabilistic %>% 
                           arrange(ID) %>%
                           dplyr::select(ID, Final1, Age:ICARTot, Control),
                        by = c("ID")
...
recover_prob5 <- foreach(pt=1:n2do, .combine = rbind) %dopar% { # multicore function for efficiency
         ...
         tryP <- as.numeric(fit_use[pt, 3:7]); 
         data <- FitRecoveredData[[pt]][c(2, 17, 18)]
      
         fitAttempt                    <- NA; # clear the decks
         fitAttempt                    <- optim(fn = wrapper_RW, #change to WSLS for last 4 parm fit
                                               par = tryP, 
                                               data = data,
                                               scbeta0 = -1
                                               )
         loglik <- NA; 
         loglik <- -wrapper_RW(fitAttempt$par, data, scbeta = NA);
         ...
         data.frame(
                    ...
                    tau    = FitRecoveredData[[pt]][1, 'tau'],
                    lrc    = FitRecoveredData[[pt]][1, 'lrc'],
                    res    = FitRecoveredData[[pt]][1, 'res'],
                    sal    = FitRecoveredData[[pt]][1, 'sal'], #add for four parms
                    mem    = FitRecoveredData[[pt]][1, 'mem'], #add for five parms
...
ggplot(PEplot %>% 
         pivot_longer(31:35, names_to = "Parameter", values_to = "Metric") %>%
         ungroup() %>%
...
FitRealBehav_df %>% 
  dplyr::select(ID, Trial, Action) %>%
  distinct() %>%
  group_by(ID, Trial > 30) %>% 
  mutate(
         count_act1 = ifelse(Action == 1, 1, 0), 
         count_act1 = sum(count_act1)/30, 
         count_act2 = ifelse(Action == 2, 1, 0), 
         count_act2 = sum(count_act2)/30, 
         count_act3 = ifelse(Action == 3, 1, 0), 
         count_act3 = sum(count_act3)/30,
         Block = ifelse(Trial > 30, 2, 1)) %>% 
  pivot_longer(count_act1:count_act3, 'actionC', values_to = 'sumaction') %>%
  ungroup() %>%
  dplyr::select(ID, actionC, sumaction, Block)%>%
  distinct() %>%
  
  ggplot(aes(factor(Block), sumaction, fill = actionC))+
  geom_boxplot(width = 0.2)+
  ggpubr::stat_compare_means()+
  theme_bw()

simplot <-  FitRealBehav_df  %>%
    as.data.frame() %>%
    dplyr::select(ID, Trial, Action, Q1, Q2, Q3, simQ1, simQ2, simQ3) %>%
    distinct() %>%
    #mutate(Q4 = ifelse(Trial > 30, Q1, Q2),
    #       Q1 = ifelse(Trial > 30, Q2, Q1),
    #       Q2 = Q4) %>%
    pivot_longer(
      c(4:9),
      names_to = "Q",
...
FitRealBehav_df %>%
  pivot_longer(31:35, names_to = "Parm", values_to = "Value") %>%
...
ModelP8 <- FitRealBehav_df %>% 
                     ...
                     distinct()
summary(lm(scale(Persec) ~ scale(tau) + scale(ICARTot) + Age + Sex + Control + sumLL,
                         data = ModelP8,
                         na.action = na.fail))
